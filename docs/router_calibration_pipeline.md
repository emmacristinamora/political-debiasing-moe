...
- **Script:** `src/router_calibration_config.py`
...
- **Script:** `src/build_router_prompt_set.py`
...
- **Script:** `src/build_router_features.py`
...
- **Script:** `src/router_calibration_utils.py`
...
- **Script:** `src/router_forced_policy_runner.py`
...
- **Script:** `src/score_router_candidates.py`
...
- **Script:** `src/build_router_targets.py`
...
- **Script:** `src/validate_router_dataset.py`
...
- **Script:** `src/split_router_dataset.py`
...
`src/train_calibrated_router.py`, optionally executes it via `subprocess`,
...
- **Script:** `src/train_router_calibration_pipeline.py`
  (wraps `src/train_calibrated_router.py`)
...
- **Script:** `src/evaluate_router_checkpoint.py`
...
```bash
python src/run_router_pipeline_smoke.py
```
...
```bash
# 1. build prompts
python src/build_router_prompt_set.py --config config/config.yaml

# 2. build features + hidden.pt (requires the base model)
python src/build_router_features.py --config config/config.yaml

# 3. run forced-policy MoCE per (prompt, candidate)
python src/router_forced_policy_runner.py --config config/config.yaml

# 4. score candidate traces
python src/score_router_candidates.py --config config/config.yaml

# 5. build target policies and write records.jsonl
python src/build_router_targets.py --config config/config.yaml

# 6. validate the records + hidden.pt
python src/validate_router_dataset.py \
    --config config/config.yaml \
    --records-path data/router/records.jsonl \
    --hidden-path  data/router/hidden.pt

# 7. split into train/val/test
python src/split_router_dataset.py --config config/config.yaml

# 8. train the calibrated-router head
python src/train_router_calibration_pipeline.py --config config/config.yaml

# 9. evaluate the trained checkpoint
python src/evaluate_router_checkpoint.py --config config/config.yaml
```
...
- **Canonical quadrant order is fixed and centralised**
  (`router_calibration_config.CANONICAL_QUADRANT_ORDER`). Every script
  imports it; the trainer duplicates it locally only because its filename
  starts with a digit, but the order matches by construction.
...
- **Trainer stays standalone.** `src/train_calibrated_router.py` is the
